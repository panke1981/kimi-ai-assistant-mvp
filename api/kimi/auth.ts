...
// ⚠️ jwks 惰性初始化 — 不在模块加载时创建，避免 KIMI_AUTH_URL 未就绪时报 Invalid URL
...
export async function authenticateRequest(headers: Headers) {
  // [staging-only] AUTH_BYPASS — 临时跳过 Kimi OAuth 认证
  // ⚠️ 仅 NODE_ENV=staging 生效，production 永不执行
  if (env.isStaging && process.env.AUTH_BYPASS === "true") {
    return {
      id: 1,
      unionId: process.env.AUTH_BYPASS_USER_ID || "staging-test-user",
      name: "测试用户",
      role: "admin" as const,
